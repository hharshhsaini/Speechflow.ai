...
from speechflow import KModel, KPipeline
from speechflow.model import KModelForONNX

def export_onnx(model, output):
    onnx_file = output + "/" + "speechflow.onnx"
    ...
    print('export speechflow.onnx ok!')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Export speechflow Model to ONNX", add_help=True)
    parser.add_argument("--inference", "-t", help="test speechflow.onnx model", action="store_true")
    parser.add_argument("--check", "-m", help="check speechflow model", action="store_true")
    parser.add_argument(
        "--config_file", "-c", type=str, default="checkpoints/config.json", help="path to config file"
    )
    parser.add_argument(
        "--checkpoint_path", "-p", type=str, default="checkpoints/speechflow-v1_0.pth", help="path to checkpoint file"
    )
    parser.add_argument(
