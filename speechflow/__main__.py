"""SpeechFlow TTS CLI
Example usage:
python3 -m speechflow --text "The sky above the port was the color of television, tuned to a dead channel." -o file.wav --debug

echo "Bom dia mundo, como vão vocês" > text.txt
python3 -m speechflow -i text.txt -l p --voice pm_alex > audio.wav
...
if TYPE_CHECKING:
    from speechflow import KPipeline


def generate_audio(
    text: str, speechflow_language: str, voice: str, speed=1
) -> Generator["KPipeline.Result", None, None]:
    from speechflow import KPipeline

    if not voice.startswith(speechflow_language):
        logger.warning(f"Voice {voice} is not made for language {speechflow_language}")
    pipeline = KPipeline(lang_code=speechflow_language)
    yield from pipeline(text, voice=voice, speed=speed, split_pattern=r"\n+")


def generate_and_save_audio(
    output_file: Path, text: str, speechflow_language: str, voice: str, speed=1
) -> None:
    with wave.open(str(output_file.resolve()), "wb") as wav_file:
        ...
        for result in generate_audio(
            text, speechflow_language=speechflow_language, voice=voice, speed=speed
        ):
